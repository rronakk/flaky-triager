...
function readQuarantineFile(path) {
...
function matchesQuarantine(suiteName, testCase, quarantineKeys) {
  const testName = testCase["@_name"] ?? "";
  const classname = testCase["@_classname"] ?? "";
  return quarantineKeys.has(`${classname}::${testName}`) || quarantineKeys.has(`${suiteName}::${testName}`);
}
function filterJunitXml(xml, quarantine) {
...
// src/filter-junit-cli.ts
var [xmlPath, quarantinePath = ".flaky-quarantine.json"] = process.argv.slice(2);
if (!xmlPath) {
...
var q = readQuarantineFile((0, import_node_path.resolve)(quarantinePath));
var targets = [];
var xmlAbs = (0, import_node_path.resolve)(xmlPath);
if ((0, import_node_fs.statSync)(xmlAbs).isDirectory()) {
  for (const f of (0, import_node_fs.readdirSync)(xmlAbs)) {
    if (f.endsWith(".xml")) targets.push((0, import_node_path.join)(xmlAbs, f));
  }
} else {
  targets.push(xmlAbs);
}
...
for (const file of targets) {
  const xml = (0, import_node_fs.readFileSync)(file, "utf-8");
  const { xml: out, skippedCount } = filterJunitXml(xml, q);
  if (skippedCount > 0) {
    (0, import_node_fs.writeFileSync)(file, out, "utf-8");
